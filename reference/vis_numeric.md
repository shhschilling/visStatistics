...
analysis (with confidence and prediction bands) or correlation analysis
(automatically selecting the most appropriate method) with appropriate
visualizations and statistical output. All statistical assumptions are
checked and warnings are issued if violated, but analysis proceeds.
...
(Shapiro-Wilk test) and homoscedasticity (Breusch-Pagan test).
Correlation: When correlation = TRUE, Spearman rank correlation is used.

All analyses proceed even if assumptions are violated, but appropriate
warnings are issued.
