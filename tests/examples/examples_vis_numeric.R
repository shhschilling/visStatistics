...
cat(
  "90% CI width for slope:",
  diff(confint(lm(y9 ~ x9), level = 0.90)[2, ]), "\n"
)
cat(
  "99% CI width for slope:",
...
summary_data <- data.frame(
  Example = c(
    "Linear (Normal)", "Quadratic", "Skewed", "Monotonic",
    "No Relationship", "Heteroscedastic", "Small Sample",
    "Missing Values", "90% CI", "Perfect"
  ),
  Regression_R2 = c(
    result1a$r_squared, result2a$r_squared, result3a$r_squared,
  ...
  Correlation_Method = c(
    result1b$method_used, result2b$method_used, result3b$method_used,
  ...
  Correlation_Coeff = c(
    result1b$correlation_coefficient, result2b$correlation_coefficient,
  ...
  Regression_Warnings = c(
    length(result1a$warnings), length(result2a$warnings),
