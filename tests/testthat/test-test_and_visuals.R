...
test_that("check_sample_size_t_test works correctly", {
  x1 <- rnorm(10)
  x2 <- rnorm(15)

  # Sufficient sample sizes
  expect_true(check_sample_size_t_test(x1, x2, 5))

  # Insufficient sample sizes
  expect_false(check_sample_size_t_test(x1, x2, 12))
})
...
test_that("odds_ratio function works correctly", {
  ...
  expect_type(result, "list")
  expect_true(nrow(result$odds_ratio_statistics) == 4) # OR, lowconf, upconf, SE
  expect_true(all(is.finite(result$odds_ratio_statistics[!is.nan(result$odds_ratio_statistics)])))
})
