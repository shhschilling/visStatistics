...
test_that("visstat: Levene robustness with non-normal data", {
  nonnormal_data <- create_nonnorm_hetero_dataset(
...
test_that("visstat: Power comparison across effect sizes", {
  ...
  medium_data <- create_heteroscedastic_dataset(
    "moderate_heteroscedastic", n_per_group = 30, seed = 401
  )
  large_data <- create_heteroscedastic_dataset(
    "severe_heteroscedastic", n_per_group = 30, seed = 402
  )
