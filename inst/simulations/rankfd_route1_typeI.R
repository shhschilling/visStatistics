## ---------------------------------------------------------------------------
## Pseudo-rank arm for the Route 1 equal-means grid.
##
...
## Continue where route1_simulations.R stopped: 150 Type I cells and 150 power
## cells, each drawing NREP streams.
CELLS_ALREADY_RUN <- 300L
  ...
  for (condition in make_conditions(mean_n)) {
    for (panel in PANELS) {
      ...
      rows[[idx]] <- data.frame(
        ...
        rank_rejection = ref,
        ps_kw_rejection = unname(res[["ps_kw"]]),
        ps_kw_mc_se = sqrt(res[["ps_kw"]] * (1 - res[["ps_kw"]]) / NREP),
        ps_ats_rejection = unname(res[["ps_ats"]]),
        ps_ats_mc_se = sqrt(res[["ps_ats"]] * (1 - res[["ps_ats"]]) / NREP),
        row.names = NULL
      )
      cat(sprintf("done: mean_n=%3d | %-38s | panel=%d | kw(saved)=%.4f ps_kw=%.4f ps_ats=%.4f\n",
                  mean_n, condition$design, panel, ref,
...
outfile <- sprintf("rankfd_route1_typeI_B%d.csv", NREP)
write.csv(result, outfile, row.names = FALSE)
message("Wrote: ", outfile)

## In the balanced designs pseudo-ranks and ranks are the same procedure, so
## the two columns must agree to within Monte Carlo error.
bal <- result[grepl("^balanced", result$design), ]
message(sprintf("balanced designs: max |ps_kw - rank_rejection| = %.5f (MC SE about %.5f)",
                max(abs(bal$ps_kw_rejection - bal$rank_rejection)),
                sqrt(2 * 0.05 * 0.95 / NREP)))
