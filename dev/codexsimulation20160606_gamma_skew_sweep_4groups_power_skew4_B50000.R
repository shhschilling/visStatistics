...
make_shift_data <- function(skew, n, shifts) {
  k <- length(shifts)
  g <- factor(rep(seq_len(k), each = n))
  y <- unlist(lapply(seq_len(k), function(i) {
    draw_gamma_or_normal(n, skew) + shifts[i]
  }))
  list(y = y, g = g)
}
...
run_cell <- function(skew, n, shifts) {
  out <- replicate(NREP, {
    dat <- make_shift_data(skew, n, shifts)
...
finalise_power <- function(power) {
  ...
  power$n_label <- factor(paste0("n = ", power$n_per_group),
                          levels = paste0("n = ", ns))
  power$effect_size <- factor(power$effect_size,
                              levels = names(SHIFT_SCENARIOS))
  power
}

write_power_checkpoint <- function(rows) {
  power <- finalise_power(do.call(rbind, rows))
...
for (n in ns) {
  for (skew in skews) {
    for (scenario_name in names(SHIFT_SCENARIOS)) {
      ...
      write_power_checkpoint(rows)
      idx <- idx + 1
    }
  }
}

power <- write_power_checkpoint(rows)
