## Fleishman Route 1 figures for the 50k simulation run.
...
OUTDIR <- file.path("dev", "codexsimulation20260615_fleishman_B50000_outputs")
...
groups <- LETTERS[seq_along(group_means)]
xlim <- c(-2.5, 5)
y_cap <- 0.7
...
p_pdf <- ggplot2$ggplot(
  ...
  ggplot2$labs(
    ...
    subtitle = "Group mean offsets: A = 0, B = 0.25, C = 0.50, D = 0.75",
...
p_power <- ggplot2$ggplot() +
  ...
  ggplot2$scale_shape_manual(values = STRATEGY_SHAPES,
                             breaks = names(STRATEGY_LABELS),
                             labels = STRATEGY_LABELS) +
  ggplot2$scale_size_manual(values = STRATEGY_SIZES,
                            breaks = names(STRATEGY_LABELS),
                            labels = STRATEGY_LABELS) +
  ggplot2$scale_colour_manual(values = STRATEGY_COLOURS,
                              breaks = names(STRATEGY_LABELS),
                              labels = STRATEGY_LABELS) +
  ggplot2$labs(
    title = "b) Power simulations",
    x = "n per group",
  ...
  ggplot2$theme(
    ...
    strip.background = ggplot2$element_blank(),
    strip.text.x = ggplot2$element_text(size = 8, face = "bold", family = "serif", hjust = 0),
    plot.title = ggplot2$element_text(hjust = 0),
    plot.title.position = "plot"
  )
...
p_parametric <- ggplot2$ggplot(
  ...
  ggplot2$scale_shape_manual(values = c("Fisher always" = 21,
                                         "Welch always" = 24,
...
ggplot2$ggsave(file.path(OUTDIR, "fleishman_4groups_power_pdf.png"),
               p_pdf, width = 14, height = 3.4, dpi = 360)
...
ggplot2$ggsave(file.path(OUTDIR, "fleishman_4groups_power.png"),
               combined, width = 14, height = 11.4, dpi = 360)
...
ggplot2$ggsave(file.path(OUTDIR, "fleishman_4groups_parametric_branch_power.png"),
               p_parametric, width = 14, height = 7.8, dpi = 360)
...
ggplot2$ggsave(file.path(OUTDIR, "fleishman_4groups_route_probability.png"),
               p_route, width = 12, height = 7.2, dpi = 360)
...
ggplot2$ggsave(file.path(OUTDIR, "fleishman_4groups_fisher_welch_route_probability.png"),
               p_fisher_welch, width = 12, height = 5.4, dpi = 360)
