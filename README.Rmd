...
The package `visStatistics` enables rapid visualization and statistical analysis of raw data by automatically selecting the hypothesis test with the highest statistical power to evaluate the relationship between a dependent variable (`varsample`) and an independent variable (`varfactor`) within a `data.frame`.
...
This fully automated workflow is particularly suited to browser-based
interfaces to server-based deployments of R and data bases.

A detailed description of the package the and its underlying decision logic, can be found in the `vignette` accompanying this package.

A graphical representation of the decision tree for a categorical feature and a numeric response is given in Figure \@ref(fig:fig-decision-tree).
