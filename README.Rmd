...
visualisation. It selects appropriate hypothesis tests for pairs of
...
In the formula interface, the formula `y ~ x`
...
- If both are factors and  `y` is additionally of class`ordered`, a non parametric test is performed: a Wilcoxon test in the case of two factor levels in `x` , a Kruskal-Wallis-Test for more than two factor levels in in `x`.
...
A graphical summary of the decision logic used for categorial predictors andnumerical responses is given in below figure: 
...
<p style="font-style: italic; font-size: 90%; margin-top: 0.5em;">
Comparing central tendencies: Decision tree used to select the appropriate statistical test for a categorical
...
## Ordered response and numerical predictor:
