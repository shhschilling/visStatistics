...
If this condition is met, the Levene-Brown–Forsythe-test [@Brown:1974] (`levene.test()`) is then used to assess homoscedasticity. When variances are homogeneous ($p > \alpha$), `aov()` is applied with Tukey's HSD (`TukeyHSD()`) for post-hoc comparison. If variances differ significantly ($p \le \alpha$), Welch's one -
