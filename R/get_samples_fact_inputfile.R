...
get_samples_fact_inputfile <- function(dataframe, varsample, varfactor) {
  ...
  if (is.null(dim(dataframe))) {
    ...
    if ("matching" %in% names(dataframe) && varfactor == "match") {
      ...
      # matchingCriteria = paste(
      #   tolower(paste(as.character(dataframe$matching), collapse = " ")),
      #   sep = ""
      # )
      matching_criteria <- tolower(paste(
        apply(dataframe$matching, 1, function(x) {
          paste(x, collapse = " ")
        }),
        collapse = ", "
      ))
      name_of_sample <- paste(name_of_sample, "with match:", matching_criteria)
