...
#' For regression, statistical assumptions are checked and warnings are
#' issued if violated, but analysis proceeds.
#'
...
#'   If empty, defaults to the variable name.
#'
#' @return A list containing analysis results and assumption checks. Content
#'   depends on analysis type.
#'   For regression analysis: analysis_type, summary_regression, assumptions,
#'   warnings, r_squared, adj_r_squared.
#'   For correlation analysis: analysis_type, correlation_test,
...
vis_numeric <- function(y,
  ...
  if (!exists("colorscheme", mode = "function")) {
    colorscheme <- function(x) {
      if (x == 1) {
        return(c("#1f77b4", "#ff7f0e"))
      } # blue, orange
      if (x == 2) {
        return(c("#2ca02c", "#d62728"))
      } # green, red
  ...
  if (!correlation) {
    ...
    if (n >= 3 && n <= 5000) {
      ...
      if (assumptions$shapiro_residuals$p.value < alpha) {
        warnings_list <- c(
          warnings_list,
          paste(
            "Normality of residuals violated (Shapiro-Wilk p =",
            signif(assumptions$shapiro_residuals$p.value, 3), ")"
          )
        )
      }
    } else {
      assumptions$shapiro_residuals <- list(
        p.value = NA,
    ...
    if (!is.na(bp_result$p.value) && bp_result$p.value < alpha) {
      warnings_list <- c(
        warnings_list,
        paste(
          "Homoscedasticity violated (Breusch-Pagan p =",
    ...
    pred_col <- "#56B4E9"
    ci_col <- "black"
    fit_col <- visstat_graphics_arg(plot_args, "col", fit_col)
    ci_lty <- visstat_graphics_arg(plot_args, "lty", 2)
    main_text <- visstat_graphics_arg(
      plot_args,
      "main",
      expression(hat(y)[i] == b[1] %.% x[i] + b[0])
    )
    ...
    plot(x_sorted, y_sorted,
      ...
      ylab = visstat_graphics_arg(plot_args, "ylab", name_of_sample),
      main = ""
    )

    polygon(c(x_sorted, rev(x_sorted)),
      c(y_pred_low, rev(y_pred_up)),
      col = adjustcolor(pred_col, alpha.f = 0.2),
      border = NA
    )

    # Add regression line and bands
    lines(x_sorted, fitted_sorted, col = fit_col, lwd = 2)
    lines(x_sorted, y_pred_low, col = pred_col, lwd = 1)
    lines(x_sorted, y_pred_up, col = pred_col, lwd = 1)
    lines(x_sorted, y_conf_low, col = ci_col, lwd = 2, lty = ci_lty)
    ...
    mtext(
      bquote(paste(
        R^2, " = ",
        .(sprintf("%.3f", reg_summary$r.squared)),
        ", conf. level = ",
    ...
    mtext(
      bquote(paste(
        "slope ", b[1], " = ",
        .(sprintf("%.2f", reg$coefficients[2])),
        ", CI [",
        .(sprintf("%.2f", conf_int_coeffs[2, 1])),
        ", ",
    ...
    mtext(
      bquote(paste(
        "intercept ", b[0], " = ",
        .(sprintf("%.2f", reg$coefficients[1])),
        ", CI [",
        .(sprintf("%.2f", conf_int_coeffs[1, 1])),
        ", ",
    ...
    legend("topleft",
      ...
      c(
        "regression line",
        sprintf(
          "%.0f%% prediction interval for an individual response",
          100 * conf.level
        ),
        sprintf(
          "%.0f%% confidence interval for the mean response",
          100 * conf.level
        )
      ),
      lwd = c(2, NA, 2),
      col = c(
        fit_col,
        adjustcolor(pred_col, alpha.f = 0.2),
        ci_col
      ),
      lty = c(1, NA, 2),
      pch = c(NA, 15, NA),
    ...
    mtext(bquote(
      "Spearman" ~ rho ~ "=" ~ .(signif(cor_coef, 3)) ~
        ", p =" ~ .(signif(p_value, 3))
    ))
  ...
  if (length(warnings_list) > 0) {
    ...
    if (!correlation && any(grepl("Normality|Homoscedasticity", warnings_list))) {
      message(
        "RECOMMENDATION: Consider exploring alternatives outside visstat() ",
        "such as data transformations,\n",
        "generalised linear models, or robust regression. For a non-causal ",
        "alternative\n",
