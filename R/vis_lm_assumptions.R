...
vis_lm_assumptions <- function(samples, fact, cex = 1, correlation = FALSE,
  ...
  # Fit model
  # Needed for the expected output structure; do not use lm(samples ~ fact).
  ...
  plot_histogram <- function(x, xlab) {
    ...
    hist(x,
      ...
      xlab = visstat_graphics_arg(plot_args, "xlab", xlab),
    ...
    legend("topright",
      legend = "normal density", col = "red", lwd = 1,
  ...
  plot_qq <- function(x, ylab) {
    ...
    plot(qq$x, qq$y,
      type = "n",
      main = visstat_graphics_arg(plot_args, "main", "QQ-Residuals"),
      xlab = "Theoretical quantiles",
      ylab = visstat_graphics_arg(plot_args, "ylab", ylab),
      ylim = y_lim
    )
    if (!is.null(qq_envelope)) {
      polygon(
  ...
  plot_residuals_vs_fitted <- function() {
    ...
    plot(fitted_values, z_residuals,
      type = "n",
      main = visstat_graphics_arg(plot_args, "main", "Residuals vs. Fitted"),
      xlab = "Fitted response",
      ylab = visstat_graphics_arg(plot_args, "ylab", "z residuals"),
      ylim = y_lim,
      yaxt = "n"
    )
  ...
  if (regression_mode) {
    visstat_graphics_par(plot_args, list(
      mfrow = c(1, 3), oma = c(0, 0, 3, 0),
      mar = c(4.5, 4, 3, 1), cex = cex
    ))
    ...
    plot(x_jitter, abs_scaled_residuals,
      main = visstat_graphics_arg(plot_args, "main", "Abs. residuals vs Group"),
      xlab = visstat_graphics_arg(plot_args, "xlab", "Group"),
      ylab = visstat_graphics_arg(plot_args, "ylab", "Absolute standardised residuals"),
    ...
    legend("topright",
      legend = "group mean", pch = 4, pt.lwd = 1.5, col = "red",
      bty = "n", cex = cex
    )
  }
  ...
  if (regression_mode) {
    ...
    title_line1 <- visstat_graphics_arg(
      plot_args,
      "main",
      paste("Shapiro-Wilk p =", p_shapiro, "| Anderson-Darling p =", p_AD)
    )
    ...
    title_line2 <- paste(
      "Levene p =", signif(levene_test$p.value, 2),
