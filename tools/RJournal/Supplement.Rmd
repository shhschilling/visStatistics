...
Student's t-test [@Student:1908] tests the null hypothesis that the means of two
...
Fisher's exact test [@Fisher:1970] (`fisher.test()`) is applied when
Cochran's rule [@Cochran:1954] is violated. The test calculates an exact
$p$-value by conditioning on the observed margins of an $R \times C$
contingency table under the null hypothesis of independence.
...
diagnostic panel. With $z_i=e_i/SE_\text{res}$, where $SE_\text{res}=\sqrt{\sum_{r=1}^{N} e_r^2/(N-k)}$, Cook's
...
| Simple linear regression | $R^2$ | $R^2 = 1 - SS_\text{res}/SS_\text{tot}$, Eq.~\@ref(eq:r-squared) | `summary(lm())$r.squared` |
