...
`levene.test()` [@Brown:1974] (see [Appendix C](#lev)) assesses
...
```{r ttest-example, fig.cap="Welch's t-test applied to the \\texttt{mtcars} dataset (\\texttt{am} vs.\\ \\texttt{mpg}). Pooled-residual assumption diagnostics (Shapiro--Wilk, Bartlett, Levene--Brown--Forsythe) and box plots of fuel efficiency by transmission type with the Welch t-test result.", out.width="48%", fig.height=4.5, fig.show="hold"}
...
group and two treatment groups [@Dobson:1983]. With control and treatment groups as predictor and
...
routing. Bootstrapping methods [@Wilcox:2021] make minimal distributional
