...
residuals versus fitted values, and normal Q--Q behaviour. The fourth
plot is a scale-location plot for grouped central-tendency analyses and a
standardised residuals versus leverage plot for simple regression.
...
Breusch--Pagan test (`bp.test()`). The regression plot shows the fitted line
...
Most example datasets are distributed with R; synthetic data are used
where no suitable base R example is available. The examples were chosen
to trigger the package branches: normal and homoscedastic residuals leading to Student's t-test or
...
It fits a linear model using `aov()` and reports the residual normality
and homoscedasticity tests used or displayed by the routing logic.
...
For simple linear regression, the Breusch--Pagan test (`bp.test()`) is implemented in the package.
