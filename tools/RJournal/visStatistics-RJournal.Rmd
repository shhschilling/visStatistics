...
abstract: >
  `visStatistics` provides an assumption-aware workflow for routine two-variable frequentist inference in R. Given two vectors, or a formula and data frame, the function determines the appropriate method from the variable classes, distributional assumptions, and sample sizes.
...
`print()` and `summary()` methods expose the complete test results,
...
belong to the general linear model framework [see Appendix A](#glm) and
...
means is assumed by the central limit theorem [@Lumley:2002;
...
regardless of whether linear-model assumptions are met, and always displays
...
```{r student-ttest-example, fig.show='hide', results='hide'}
...
$\hat{y} = b_0 + b_1 x$, where $b_0$ is the intercept and $b_1$ the
...
estimates of the confidence and prediction bands. The regression is
reported regardless of the diagnostics; in this branch, `visstat()`
therefore also serves as a model-triage tool.
...
The `airquality` ozone example illustrates this triage role. A default
`visstat()` call for ozone concentration (`Ozone`) as a function of wind
speed (`Wind`) fits the simple linear model, but the diagnostics flag
violated residual normality and homoscedasticity. The printed
recommendation suggests two possible next steps: exploring alternatives
outside `visstat()`, such as data transformations, generalised linear
models, or robust regression, or rerunning the analysis with
`correlation = TRUE` for an assumption-free, non-causal Spearman
analysis. For these data, a Gamma GLM with log link is one plausible
follow-up outside `visstat()` because `Ozone` is positive, continuous,
and shows variance increasing with the mean.
