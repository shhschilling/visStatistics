...
simple-regression branch, this residual check is also the first visual
output: `vis_lm_assumptions()` displays a four-panel diagnostic graphic
before the selected test or regression result (see
[Appendix B](#assumptions)). The panel shows the residual distribution,
residuals versus fitted values, normal Q--Q behaviour, and, depending on
the branch, scale-location or leverage diagnostics.
...
The first split checks group size. The diagnostic panel is generated
before this split is evaluated. When all groups contain more than 50
...
Welch's `oneway.test()` with `games.howell()` ([@Games:1976]; see [Appendix C](#gh)) for
...
Breusch--Pagan test (`bp.test()`; see [Appendix C](#bp)). The regression plot shows the fitted line
...
t-test (Figure \@ref(fig:student-ttest-example)).
...
(Figure \@ref(fig:wilcoxon-example)).
...
(Figure \@ref(fig:ordinal-wilcoxon-example)).
...
$\alpha = `r example_alpha`$, and the Tukey HSD post-hoc comparison finds no significant difference between the control group and either treatment, but the difference between `trt1` and `trt2` is significant (Figure \@ref(fig:anova-example)). 
...
comparisons (Figure \@ref(fig:welch-anova-example)).
...
The resulting Kruskal--Wallis output is given in
Figure \@ref(fig:ordinal-kruskal-example).
...
(Figure \@ref(fig:regression-example)).
...
(Figure \@ref(fig:ozone-lm-triage)).
...
both a grouped bar chart of row percentages and a mosaic plot with tiles giving the count per cell coloured by Pearson residuals. The mosaic plot shows which cells drive
...
correction; Figure \@ref(fig:hair-eye-2x2-wide-panel) contrasts this
route with Fisher's exact test for a different slice of the same
contingency table.

```{r hair-eye-2x2-wide-panel-data, include=FALSE}
if (!requireNamespace("png", quietly = TRUE)) {
  stop("The png package is required to build this panel figure.")
}
panel_file <- "figures/hair-eye-2x2-wide-panel.png"
panel_a <- tempfile(fileext = ".png")
panel_b <- tempfile(fileext = ".png")

png(panel_a, width = 900, height = 700, res = 120)
...
png(panel_b, width = 900, height = 700, res = 120)
...
img_a <- png::readPNG(panel_a)
img_b <- png::readPNG(panel_b)
png(panel_file, width = 1800, height = 700, res = 120)
par(mar = c(0, 0, 0, 0))
plot.new()
plot.window(xlim = c(0, 1), ylim = c(0, 1), xaxs = "i", yaxs = "i")
rasterImage(as.raster(img_a), 0, 0, 0.495, 1)
rasterImage(as.raster(img_b), 0.505, 0, 1, 1)
dev.off()
```

```{r hair-eye-2x2-wide-panel, echo=FALSE, out.width="100%", fig.cap="Two $2 \\times 2$ analyses of subsets of the \\texttt{HairEyeColor} dataset. A: Pearson's $\\chi^2$ test with Yates' continuity correction for a $2 \\times 2$ subset of \\texttt{HairEyeColor} (black/brown hair and brown/blue eyes). Grouped bar chart of row percentages with the corrected test result. B: Fisher's exact test applied to a $2 \\times 2$ subset of \\texttt{HairEyeColor} (male participants, black/brown hair, hazel/green eyes). Bar chart of absolute counts with the $p$-value."}
...
the $p$-value (Figure \@ref(fig:kendall-example)).
...
The first plot is a histogram of the standardised residuals with a
standard normal density overlay. The remaining plots are inherited from
`plot.lm()`: residuals against fitted values, a normal Q--Q plot, and
either a scale-location plot or, for numeric predictors with
`correlation = FALSE`, a standardised residuals vs. leverage plot with
