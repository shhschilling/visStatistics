...
statistics and association measures are collected in [Appendix C](#tests).
...
For more than two groups, this route leads to `kruskal.test()`,
Fisher's one-way ANOVA (`aov()`), or Welch's heteroscedastic one-way
ANOVA (`oneway.test()`). The graphical output consists of two panels. The
...
```{r anova-example, fig.cap=paste0("Fisher's one-way ANOVA applied to the \\texttt{npk} dataset (\\texttt{block} vs.\\ \\texttt{yield}). Assumption diagnostics and box plots of pea yield by experimental block; Tukey HSD post-hoc comparisons are shown as compact-letter labels ($\\alpha = ", example_alpha, "$)."), out.width="48%", fig.height=4.5, fig.show="hold"}
...
```{r welch-anova-example, fig.cap=paste0("Welch's heteroscedastic one-way ANOVA applied to the \\texttt{iris} dataset (\\texttt{Species} vs.\\ \\texttt{Sepal.Length}). Assumption diagnostics, group-wise normality diagnostics, and box plots; Games--Howell post-hoc comparisons are shown as compact-letter labels ($\\alpha = ", example_alpha, "$)."), out.width="32%", fig.height=4.5, fig.show="hold"}
...
```{r ordinal-kruskal-example, fig.cap=paste0("Kruskal-Wallis test for ordered comfort ratings by customer segment. The ordered response is converted to integer level codes; assumption diagnostics and box plots show the Kruskal-Wallis result; Holm-adjusted pairwise Wilcoxon post-hoc comparisons are shown as compact-letter labels ($\\alpha = ", example_alpha, "$)."), out.width="48%", fig.height=4.5, fig.show="hold", results='hide'}
set.seed(123)
