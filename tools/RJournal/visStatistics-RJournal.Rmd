...
```{r welch-anova-example, fig.cap="Welch's heteroscedastic one-way ANOVA applied to the \\texttt{iris} dataset (\\texttt{Species} vs.\\ \\texttt{Sepal.Length}). Assumption diagnostics, group-wise normality diagnostics, and box plots with Games--Howell post-hoc comparisons.", out.width="32%", fig.height=4.5, fig.show="hold"}
...
```{r ozone-lm-triage, fig.show='hide', results='hide'}
...
The second path is model exploration outside `visstat()`. For these
data, a Gamma GLM with log link is one plausible follow-up because
`Ozone` is positive, continuous, and shows variance increasing with the
mean.
