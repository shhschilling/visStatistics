...
When selecting tests for group comparisons, packages with related scope
such as `automatedtests` [@Zeevat:2025], `autotestR`
[@autotestR:2026], `boxTest` [@boxTest:2026], and `compareGroups`
[@Subirana:2014] differ in scope and in how normality is used for
routing. In these packages, normality decisions are based solely on the
input numeric vectors. This multiplies normality decisions across
variables, whereas `visStatistics` bases the central-tendency branch on
pooled standardised residuals from the common linear model, adhering to
the general linear model framework [@Searle:1971].

`compareGroups` is primarily designed for bivariate descriptive tables
and reports [@Subirana:2014]. It is therefore related, but not a direct
competitor for an inspectable test-selection workflow. `boxTest` covers
only the two-group numeric-response case, selecting between an
independent two-sample t-test and a Mann--Whitney U test after group-wise
Shapiro--Wilk tests [@boxTest:2026]. `autotestR` is broader, providing
automated functions for basic tests, including t-tests, ANOVA,
correlation, and contingency-table analyses [@autotestR:2026]. Its scope
is closer to `visStatistics` than `compareGroups`, but it is organised as
a collection of test-specific functions rather than a single
two-variable routing interface.
...
regression-type branches, `automatedtests` constructs a model from the
input variables, but does not provide the corresponding model-assumption
workflow.
...
```{r ordinal-wilcoxon-example, fig.cap="Wilcoxon rank-sum test for ordered passenger class by sex in the expanded \\texttt{Titanic} data. Ordered class is converted to integer level codes; assumption diagnostics and box plots show the Wilcoxon result.", out.width="48%", fig.height=4.5, fig.show="hold", results='hide'}
...
`bartlett.test()` and `bp.test()` have standalone implementations in the package to avoid dependencies on external packages (see [Appendix C](#variance)).
