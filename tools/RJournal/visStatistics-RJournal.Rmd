...
rule-of-thumb threshold to bypass residual-normality testing.

Simulation studies suggest that Shapiro--Wilk has the
...
route between mean-based and rank-based methods when all groups contain
fewer than 50 observations. For larger groups, however, Shapiro--Wilk,
as all normality tests, becomes overly sensitive and can reject residual
normality for deviations that are not decisive for mean-based inference.

The Anderson--Darling test gives more weight to tail deviations
[@Anderson:1952; @Razali:2011]. This tail sensitivity makes it useful as
an additional diagnostic when the residual distribution is inspected
visually; it is therefore reported in the assumption plots but does not
drive the routing decision.
