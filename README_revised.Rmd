...
A minimal function call to `visstat()` can use either of two forms:

```r
# Standardised usage (recommended)
visstat(dataframe$feature, dataframe$response)

# Backward-compatible usage
visstat(dataframe, "feature", "response")
```

In both cases, the arguments must refer to variables of class `"numeric"`, `"integer"`, or `"factor"`. If one variable is numeric and the other a factor, the numeric must be passed as the second argument (the response), and the factor as the first (the feature). If both are numeric, a linear model is fitted. If both are factors, an association test is used. The statistical test remains the same regardless of order in the categorical–categorical case, but the visual output will differ.
r
