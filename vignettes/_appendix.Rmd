...
denote the random upper-left cell count. Conditional on the margins, $A$
follows Fisher's noncentral hypergeometric distribution

\begin{equation}
\operatorname{P}_{\theta}(A=k \mid a+b,c+d,a+c,b+d)=
\frac{w_k\,\theta^{k}}{\sum_{j} w_j\,\theta^{j}},
\qquad
w_j=\dbinom{a+b}{j}\dbinom{c+d}{a+c-j},
(\#eq:fisher-noncentral)
\end{equation}

of which Eq. \@ref(eq:fisher-hypergeom) is the case $\theta=1$. Differentiating
its log-likelihood $\ell(\theta)=a\log\theta-\log\sum_{j} w_j\theta^{j}$ with
respect to $\theta$ gives
$\bigl(a-\operatorname{E}_{\theta}(A \mid a+b,c+d,a+c,b+d)\bigr)/\theta$, so
$\hat\theta_{\mathrm{cond}}$ is the value of $\theta$ at which this derivative
vanishes, that is the solution of
