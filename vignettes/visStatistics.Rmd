...
The choice of statistical test performed by the core function visstat() depends on the variable types, number of factor levels, and distributional assumptions. For example, it may use a t-test to compare group means assuming normality, or a Wilcoxon rank-sum test to compare the ranks of two groups without distributional assumptions. For more than two groups, it may use ANOVA or the Kruskal–Wallis test. Numeric-numeric relationships are analyzed using linear regression, and categorical associations with the ${\chi}^2$- or Fisher’s exact test, depending on cell counts.
...
A graphical representation of the decision tree for a categorical feature and a numeric response is given in Section \@ref(sec-decisiontree),Figure \@ref(fig:fig-decision-tree).


**Note:**  
Except for the user-adjustable `conf.level` parameter, all statistical tests are applied using their default settings from the corresponding base R functions (e.g., `t.test()`).

As a consequence, paired tests are not currently supported. Furthermore, since the main purpose of this package is to **visualize** statistical test results, only simple linear regression is implemented.

# Decision Logic for Selecting Tests of Central Tendency {#sec-decisiontree}

If the feature consists of `class` "`factor`" with two or more levels and the response consists is of `class` "`numeric`" or "`integer`" (both having mode "`numeric`"), statistical tests are applied to compare the central tendencies across groups.
A graphical overview of the decision logic used to select statistical tests for comparing central tendencies is provided in Figure \@ref(fig:fig-decision-tree).
This section explains the conditions under which parametric and non-parametric tests are chosen, based on the response type, the number of factor levels, and the underlying distributional assumptions.
...
When the feature has exactly two levels, Welch's
t-test is used to compare group means if normality assumptions hold; otherwise, the Wilcoxon rank sum test is applied (see Figure \@ref(fig:fig-decision-tree)).
Welch's
t-test evaluates the null hypothesis that the two groups have equal means.
Unlike Student`s
t-test, which assumes equal variances (homoscedasticity), Welch`s
test does not require this assumption.
Welch's
t-test exhibits minimal loss of robustness compared to Student`s
t-test, even when the latter's
assumptions are satisfied [@Moser:1992fp; @Delacre:2017iv]; therefore, Student's
t-test is not implemented.
...
-   If the sample size in both groups exceeds 30, the function always performs Welch's *t*-test (`t.test()`) is always performed . For large samples, Welch`s *t*-test is robust across a variety of conditions, even when normality is not strictly met [\@ @Rasch:2011vl; @Lumley2002dsa].
-   If at least one group has fewer than 30 observations, the function applies the Shapiro-Wilk normality test to both groups:
    -   If both $p$-values exceed the significance level $\alpha = 1-$`conf.level`, it performs Welch`s
        *t*-test (`t.test()`).
...
WhenWelch`s
*t*-test is applied, the function also includes confidence intervals at the specified `conf.level`.
...
As an example, we use the *Motor Trend Car Road Tests* data set (`mtcars`), which contains 32 observations.
In the example below, `mpg` denotes miles per (US) gallon, and `am` represents the transmission type (`0` = automatic, `1` = manual).
...
If the feature is of `class` "`factor`" with more than two levels and the response is of `mode` "`numeric`", `visstat()` initially attempts an analysis of variance (ANOVA).
ANOVA is performed only if both of the following null hypotheses cannot be rejected at the specified `conf.level`:
...
If only the assumption of normality is satisfied, `visstat()` applies Welch's
one-way test (`oneway.test()`).
If the normality assumption is violated, a Kruskal-W
allis test (`kruskal.test()`) is used instead.
...
The `visAnovaAssumptions()` function assesses the normality of standardized residuals from the ANOVA fit using both the Shapiro-W
ilk test (`shapiro.test()`) and the Anderson-D
arling test (`ad.test()`).
Normality is assumed if at least one of the two tests yields a $p$-value greater than $1 -$ `conf.level` (see Figure \@ref(fig:fig-decision-tree)).
...
Homoscedasticity is assessed using Bartlett`s
test (`bartlett.test()`), which tests the null hypothesis that the variances across all levels of the grouping variable are equal.
...
-   Otherwise, homoscedasticity cannot be assumed, and the function reports the $p$-value fromWelch`s one-way test (`oneway.test()`).
...
If the Holm-adjusted $p$-value for a given pair of groups is smaller than the significance level $1 -$ `conf.level`, the green letters displayed below the corresponding box plots will differ.
...
Normality of residuals is supported by graphical diagnostics (scatter plot of standardized residuals, Q-Q plot) and formal tests (Shapiro–Wilk and Anderson–Darling, both with $p > \alpha$).
...
If both $p$-values are below the significance level $\alpha = 1 -$conf.level, `visstat()` switches to the non-parametric `kruskal.test()`.
...
Note that multiple linear regression is **not** implemented, as the main focus of `visstat()` is the visualisation of statistical tests.

If the feature `varfactor` and the response `varsample` contain only one level each and both are of type `numeric` or `integer`, `visstat()` performs a simple linear regression.
...
`visstat()` checks the normality of the standardised residuals from `lm()` both graphically and using the Shapiro–Wilk and Anderson–Darling tests (see also section [Residual analysis](#aov_residuals)).
...
Both the graphical analysis of the standardised residuals and $p$-values greater than $1 -$conf.level in the Anderson–Darling and Shapiro–Wilk tests suggest that the assumption of normally distributed residuals is met.
...
If both the feature `varfactor` and the response `varsample` are categorical variables of type `factor`, `visstat()` tests the null hypothesis that the two variables are independent.

Categorical data are typically represented as contingency tables—multidimensional arrays containing the observed frequencies for each unique combination of feature and response levels.
Based on these observed frequencies, `visstat()` computes the expected frequencies under the null hypothesis.

If the expected frequencies are sufficiently large —defined as at least 80% greater than 5 and none less than 1—the `chisq.test()` is used.
Otherwise, `visstat()` switches to `fisher.test()` [@Cochran].
In the case of 2-by-2 contingency tables, a continuity correction is applied in the `chisq.test()`.

`visstat()` produces a grouped column plot with the test's $p$-value in the title, along with a mosaic plot showing Pearson's residuals (see the documentation of `mosaic()` in the `vcd` package for details).

`visstat()` requires input in the form of a `data.frame` with a column-based structure.
Contingency tables can be converted to this format using the helper function `counts_to_cases()`.
This proceduce will be exemplified with the data set `HairEyeColor`.
...
For 2-by-2 contingency tables, `visstat()` applies the continuity correction when computing the ${\chi}^2$-squared test statistic. In the following example, we restrict the data to participants with either black or brown hair and either brown or blue eyes, resulting in a 2-by-2 contingency table.
...
The generated graphics can be saved in any file format supported by `Cairo()`, including "png", "jpeg", "pdf", "svg", "ps", and "tiff". 
In the following example, we store the graphics in PNG format in the `plotDirectory` `tempdir()`. The file names reflect the statistical test used and the variable names involved.
