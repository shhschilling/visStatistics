...
<!-- The residual standard error $SE_\text{res}$ is a *global* estimate for the unknown $\sigma$, but  -->
<!-- $SE_\text{res}^2$ is not the best estimate for the variance of the *individual* residual $\operatorname{Var}(e_i)$ -->

<!-- It can be shown [@Cook:1982, p. 14; @Keller:2026] that -->
<!-- \begin{equation} -->
...
<!-- where  -->
<!-- the leverage \(h_i\) of observation \(i\) measures how strongly its predictor -->
<!-- values influence its fitted value \(\hat{y}_i\). -->
<!-- From Eq. \@ref(eq:var-leverage) follows that  -->
<!-- observations with higher -->
<!-- leverage have -->
<!-- smaller individual residual under constant variance. Internally -->
<!-- studentised residuals or "standardised" residuals correct for that. Dividing    -->
<!-- $e_i$ by its estimated individual standard error leads to  -->
<!-- \begin{equation} -->
<!-- r_i =\frac{e_i}{\sqrt{ SE_\text{res}^2\cdot (1-h_i)}}= -->
<!-- \frac{z_i}{\sqrt{1-h_i}}. -->
<!-- \end{equation} -->
...
**Normality tests** The normality of the residuals is formally assessed using both the Shapiro--Wilk test [@Shapiro:1965; @Royston:1982; @Royston:1995] (`shapiro.test()`; Eq. 
...
As normality tests asses shape, testing on the raw residuals (Eq. \@ref(eq:raw-residual)) or z-residuals (Eq. \@ref(eq:z-residual)) results in the same test statistics.
...
**Route 1** For numeric responses with categorical predictors, the diagnostic panel displays the residual histogram, the normal Q--Q plot, and the absolute z-residuals $|z_i|$ (Eq. \@ref(eq:z-residual)) by group.
...
The first row of the outer title reports p-values of residual-normality checks with the Shapiro--Wilk test and Anderson--Darling tests.
...
When all group-specific sample sizes are greater than 50, the sampling distribution of the group means is treated as sufficiently close to normal by the central limit theorem [@Lumley:2002]. In this case, the test selection is independent of the residual-normality p-values displayed in the diagnostic panel (Setion \@ref(sec:graphical)).
...
If the normality of the model residuals is not rejected or its assumption checking is bypassed to large sample size justificed by the central limit theorem, their variance homogeneity is assessed with the package-implemented mean-centred Levene test (L) [@Levene:1960] (Eq. \@ref(eq:levene-f)).
...
Many routine statistical analysis reduce to a relatively small number of tests  [@]. Under those tests parametric tests like t-tests, analysis ov variance or regression belong to the family of general linear models, which's  assumptions are frequently not tested at all or not tested properly [@Kerry:2003] 


`visStatistics`  addresses this gap: Its automated test selection relying on the data type, size and p-values of assumption tests is fully transparent, but assess the inherent shortcomings of test selection based on the p-values by assumptions tests [@ ]  by supplementing the output with diagnostic plots of the assumption tests. 
