...
standardised residuals from a linear model fitted to all groups. `autotestR` and
...
`x` and `y` must be vectors of class `"numeric"`, `"integer"`, or `"factor"`. The `visstat()` parameter `conf.level` defaults to 0.95, equivalent to a default significance level of $\alpha = 0.05$.
...
`visStatistics` is available on CRAN. This vignette corresponds
to the current GitHub version (https://github.com/shhschilling/visStatistics),
which may differ slightly from the latest CRAN release. The main
function `visstat()` returns an object of class `"visstat"` with
`print()`, `summary()`, and `plot()` methods. The returned object also
contains an `effect_size` field with the effect-size name, numeric estimate,
...
(package-implemented `levene.test()`; [@Brown:1974]; see [Appendix C](#lev)). For brevity,
...
([@Games:1976]; see [Appendix C](#gh)) for more than two groups.
...
The box plots are enriched with significant-letter labels to visualise the post-hoc analysis:
...
significant-letter labels below the box plots; pairs sharing a letter are not
...
package-implemented Breusch--Pagan test (`bp.test()`; see [Appendix C](#bp)). The regression plot shows the fitted line
...
from `vcd` [@Meyer:2006; @Meyer:2024] with tiles coloured by
...
For two groups, `visstat()` routes to Student's t-test, Welch's t-test,
or Wilcoxon rank-sum test following the decision logic above.
...
```{r anova-example, fig.cap=paste0("Fisher's one-way ANOVA applied to the `PlantGrowth` dataset (`group` vs.\\ `weight`). Assumption diagnostics and box plots of plant weight by treatment group; Tukey HSD post-hoc comparisons are shown as significant-letter labels ($\\alpha = ", example_alpha, "$)."), out.width="48%", fig.height=4.5, fig.show="hold"}
...
```{r welch-anova-example, fig.cap=paste0("Welch's heteroscedastic one-way ANOVA applied to the `iris` dataset (`Species` vs.\\ `Sepal.Length`). Assumption diagnostics and box plots; Games--Howell post-hoc comparisons are shown as significant-letter labels ($\\alpha = ", example_alpha, "$)."), out.width="48%", fig.height=4.5, fig.show="hold"}
...
analysis with `correlation = TRUE` for a rank-based, non-causal
...
Correlation analysis requires the explicit flag `correlation = TRUE`. For the ozone example,
...
As a model outside of  `visstat()`, we fit a Gamma generalised linear model with log link. The Gamma family is suited here because Ozone is strictly positive and continuous, and its variance grows with the mean — the structure detected by the Breusch--Pagan test. The log link guarantees positive fitted values.
...
The Gamma GLM significantly improves the model fit, as evidenced by the drop in the Akaike Information Criterion [@Akaike:1974] from 1093.2 to 1040.0. The increase in the Shapiro–Wilk $p$-value from $p_{SW} = 0.0052$ in the simple linear regression to $p_{SW} = 0.78$ indicates that the Gamma GLM successfully corrected the model misspecification. Since the standardised deviance residuals no longer show significant departures from normality, the heteroscedasticity is successfully handled. This comparison demonstrates that when `visstat()` signals a failure of the Gaussian framework, moving to a GLM provides a statistically superior and valid alternative.
...
## Ordered response, ordered predictor

When both variables are ordered factors and `correlation = TRUE`,
...
inflates the overall Type I error rate. Based on the provided
diagnostic plots, the user might overwrite the automated choice of test in individual cases.

The reported effect size should be interpreted alongside the \(p\)-value, but it
...
The $n > 50$ threshold (see [Decision logic](#routing)) is motivated by
simulation studies showing that moderately skewed distributions require
roughly 40--50 observations for adequate convergence of the sampling
distribution of the mean [@Lumley:2002; @Fagerland:2012]. For smaller
samples, Shapiro--Wilk has the highest power among normality tests for
small to moderate sample sizes [@Razali:2011].
...
sizes, and plots that should inform that judgment. The package therefore serves as a transparent entry point for routine
...
The Shapiro--Wilk test evaluates whether a sample
...
The implementation uses `ad.test()` from `nortest`
[@Gross:2015].
...
Brown--Forsythe Levene-type test when normality holds [@Allingham:2012]. Its test
statistic is
...
Welch's t-test (`t.test(..., var.equal = FALSE)`) relaxes the homoscedasticity assumption. Its statistic is
...
`wilcox.test()` is the Mann--Whitney $U$ statistic [@Mann:1947] of the
first group:
...
$F \sim F(k-1, N-k)$. Post-hoc comparisons use Tukey's Honest Significant Differences
`TukeyHSD()`, which controls the family-wise error rate.
...
The degrees of freedom use the Welch--Satterthwaite equation given
above. Two-sided $p$-values are computed from the corresponding
...
When normality is rejected, `kruskal.test()` tests
...
Yates' continuity correction is applied by default. For
