...
3. **Homoscedasticity** - constant variance of residuals ($\sigma^2$ is constant)
4. **Normality of residuals** - $\boldsymbol{\varepsilon} \sim N(\mathbf{0}, \sigma^2\mathbf{I})$

<!-- Importantly, it is the **residuals** (not the raw data in each group) that must be approximately normally distributed. -->
The function `vis_anvaa_assumption() `checks these assumptions through the standardized residuals, which pool information from aall observations and have $N - k$ degrees of freedom (where $N$ is the total sample size and $k$ is the number of parameters or groups).
...
It  provides diagnostic plots (Q-Q plots, residual vs. fitted values and histograms of residuals ) alongside formal tests (Shapiro-Wilk, Anderson-Darling for testing the normality of the residuals, Levene-Test and Bartlett test for testing homoscedacity) to help users assess whether the assumptions of the GLM are reasonably met. 
...
Under the null hypothesis that all group variances are equal and the data are 
normally distributed, the test statistic approximately follows a 
