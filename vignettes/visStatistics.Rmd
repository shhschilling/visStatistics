...
While numerous R packages provide statistical testing functionality, few are designed with pedagogical accessibility as a primary concern. The `visStatistics` package addresses this challenge by automating test selection based on deterministic decision logic, removing the burden of manual test selection. This automation enables users to directly engage with the interpretation of statistical outcomes rather than being bogged down in the test selection process. The tailored visual outputs, annotated with test results and, where appropriate assumption checks and post-hoc analysis  further aid comprehension and ensure that valid statistical conclusions are drawn from the outset. The package is particularly valuable in statistical consulting for student research projects, where time constraints make streamlined, assumption-aware output essential for focusing on interpretation rather than technical execution. The implemented tests cover the typical content of an introductory course in statistcs on undergraduate leve. 


The package also suits server-based applications where users have limited interaction: they provide only input vectors, and the software returns valid, interpretable results without requiring further statistical knowledge. This supports reproducibility and correct inference even in constrained environments.
...
sampling distribution of the mean [@Rasch:2011; @Lumley:2002;@Kwak:2017]. Welch's t-test is
the default method in R for comparing means and is generally
preferred over Student's t-test because it does not assume equal variances.
It maintains comparable power even when variances are equal and outperforms
Student's test when variances differ [@Moser:1992; @Fagerland:2009; @Delacre:2017].
...
-   For predictors with more than two levels, a model of Fisher's one-way analysis of variables (ANOVA) (`aov()`) is initially fitted.
    <!-- The normality of residuals is evaluated by the  the Shapiro--Wilk test (`shapiro.test()`) and the Anderson-Darling test (`ad.test()`); residuals are considered approximately normal if at least one of the two tests yields a result exceeding the significance threshold $\alpha$. -->
     The normality of residuals is evaluated by the the Shapiro--Wilk test (`shapiro.test()`); residuals are considered approximately normal if it yields a result exceeding the significance threshold $\alpha$.
...
The `visAnovaAssumptions()` function assesses the normality of standardised residuals from the the ANOVA fit using both the Shapiro--Wilk test (`shapiro.test()`) and the Anderson--Darling test (`ad.test()`). This approach leverages their complementary strengths: Shapiro-Wilk generally demonstrates superior power for diverse non-normal distributions in smaller samples, while Anderson-Darling is highly sensitive to tail deviations and performs robustly with larger sample sizes [@Razali:2011; @Yap:2011].
...
The algorithmic decision logic cannot replicate the combination of formal tests 
and expert visual assessment of normality. It therefore assumes normality if the
Shapiro–Wilk test yields a p-value greater than $\alpha$, as simulation studies
have shown it to offer the highest power against a variety of non-normal
distributions in small to moderate samples [@Razali:2011].
