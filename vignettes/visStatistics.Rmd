...
Because no single assumption test is definitive across sample sizes and distributional deviations, these diagnostics are reported for inspection rather than treated as proof.
...
- Regardless of sample size, assumption diagnostics are always displayed.
  ...
  The decision logic is based on p-values and distributional assumptions; users should also inspect the returned `effect_size` field when interpreting practical relevance [@Shatz:2024].
...
In the linear regression branch, homoscedasticity is assessed using the Breusch–Pagan test `bp.test()` (see [Appendix C](#variance)), which evaluates whether the variance of the raw residuals depends on the predictor variable.
...
These three tests have standalone implementations in the package to avoid dependencies on external packages (see [Appendix C](#variance)).
...
```{r wilcoxon-example, fig.show='hide', results='hide'}
wilcoxon_warpbreaks <- visstat(warpbreaks$wool, warpbreaks$breaks)
```

#### Wilcoxon with ordered response {.unnumbered}
...
#### Welch's heteroscedastic one-way ANOVA {.unnumbered}
...
```
The Gamma generalised linear model significantly improves the model fit, as evidenced by the drop in Akaike Information Criterion (AIC, [@Akaike:1974])  from 1093.2 (simple linear model) to 1040.0  
...
#### Scope and defaults {#limitations .unnumbered}
...
settings from the corresponding base R functions. As a consequence,
paired tests are currently not
supported and `visstat()` does not support interaction terms
...
assumption tests. However, no single assumption test maintains optimal Type I
...
# Appendix B: Tests for normality {#normality}
...
The implementation uses `shapiro.test()` from `stats` [@R:2026].
...
and $\Phi$ the standard normal cumulative density function. The test statistic is
...
[@Gross:2015]. Although the Anderson--Darling result is shown in all assumption
plots, it does **not** drive the branching decision; only Shapiro--Wilk
does.

# Appendix C: Tests for homoscedasticity {#variance}
