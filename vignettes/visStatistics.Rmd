...
selected test and an `effect_size` field (Section
\@ref(sec:p-values-effect-sizes)).
...
<!-- Note that in this large sample case  -->
<!-- the p-value of the  Shapiro--Wilk (SW) test [@Shapiro:1965; Eq. \@ref(eq:shapiro-w)] does not decide routing. -->


Otherwise, the p-value of the Shapiro--Wilk (SW) test [@Shapiro:1965; Eq. \@ref(eq:shapiro-w)]
determines whether the route remains mean-based: 
If normality is rejected ($p_\text{SW} \le \alpha$), rank-based tests are selected: `wilcox.test()` (Eq. \@ref(eq:wilcoxon-w)) for two groups, or `kruskal.test()` (Eq. \@ref(eq:kruskal-h)) followed by Holm-adjusted `pairwise.wilcox.test()` for more than two groups.
...
implemented formulae are given in Appendix \@ref(sec:effect-size).
...
\@ref(sec:effect-size) [@Fritz:2012; @Levine:2002].
...
Section \@ref(sec:examples-rank-correlation-mode).
...
\@ref(fig:yates-fisher-example), right).
...
For the ozone example introduced in Section \@ref(sec:lin-reg), staying within
