...
The title is structured as follows: 
 ...
 - Third line: Test statstic, p value and automated comparison with $\alpha$
...
##  ANOVA, One-way test and Kruskal-Wallis test 
...
In the remainder of this section we will briefly introduce the tests, explain the the test choice and demonstrate how the assumptions are tested.
and illustrate each tests by an example.
...
<h4> Welch's One-Way test </h4>
When the assumptions of homoscedasticity is not met, but the assumptions of
independent observations and normally distributed residuals are ,et. 
...
###  Test choice
From the above assumptions for the different tests, follows directly the test logic: 
 `visstat()` initially attempts an analysis of variance (ANOVA) as implemented in `aov()`.
...
$a$ is the intercept and $b$$ its slope.
...
expected, while negative values suggest underrepresentation (shaded in red).
...
For all tests of independence  `visstat()` displays a grouped column plot that includes the respective test's $p$-value in the title, as well as a mosaic plot showing color-coded Pearson residuals and the p-value of Pearson's $\chi^2$-test.

## Transforming a contingency table to data frame
 The following examples for tests of categorical feature and response are all based on the `HairEyeColor` contingency table. 
 
Contingency tables must get converted to the required column-based `data.frame` using the helper function `counts_to_cases()`. The function transforms the contingency table `HairEyeColor` into `data.frame` named `HairEyeColorDataFrame`.
...
Note the higher p-value of  Pearson's ${\chi}^2$-test with Yate's continuity correction (p=0.00354) compared to the p-value of Perasons  ${\chi}^2$-test (p=00229)shown in the mosaic plot. 
