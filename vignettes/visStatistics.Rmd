...
`visStatistics` is available on CRAN as the latest stable
release. This vignette refers to the latest development state in the
GitHub repository (https://github.com/shhschilling/visStatistics),
which may include minor changes between CRAN submissions. The main
...
$\alpha = `r example_alpha`$, and the Tukey HSD post-hoc comparison finds no significant difference between the control group and the treatment, but the difference between `trt1` and `trt2` is significant.
