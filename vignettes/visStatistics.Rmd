...
Assessment for normality should never solely rely on p-values, but should be complimented by visual inspections.
Therefore the function generates two diagnostic plots: - a scatter plot of the standardised residuals against the fitted means of the linear model for each level of the predictor, and - a Q--Q plot of the standardised residuals.
...
The `cars` dataset reports the speed of 50 cars in miles per hour (`speed`) and the stopping distance in feet (`dist`) recorded in  the 1920s. In this example the assuptions of normally distributed residuals in not met
