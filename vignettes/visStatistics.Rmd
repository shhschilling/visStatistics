...
author: 
  - name: "Sabine Schilling"
    affiliation: "Institue of Tourism and Mobility, Lucerne Unviersity of Applied Sciences and Arts"
    email: "sabine.schillingg@protonmail.com"
date: "`r Sys.Date()`"
output:
 #html_vignette: #for CRAN, much more lightweight 
 bookdown::html_document2:  #for correct disply in pkdown of section numbering
# pdf_document:
    toc: true
    number_sections: true
    toc_depth: 3
link-citations: yes
pkgdown:         #these two lines are needed for building the package site correctly
  as_is: true
...
-   Section 3 focues on the installation and the main function call,
...
Welch's t-test remains valid and exhibits only minimal loss of power even when the assumptions of Student's t-test -- namely, normality and equal variances of the response variable across groups -- are satisfied [@Moser:1992; @Delacre:2017]. It is therefore the default implementation of the t-test in R. be .
...
This approach leverages their complementary strengths: Shapiro-Wilk generally demonstrates superior power for diverse non-normal distributions in smaller samples, while Anderson-Darling is highly sensitive to tail deviations and performs robustly with larger sample sizes [@Razali:2011; @Yap:2011].

<!-- This strategy increases tolerance to minor deviations from normality and reduces the risk of false positives (Type I errors for non-normality detection), but may increase the risk of false negatives (Type II errors), potentially overlooking meaningful departures from normality[@Ghasemi:2012]. -->
Statistical tests on assumptions are generally not
very reliable, sometimes breaking down in both small and large samples - the Shapiro-Wilk-tests breaks down on large samples ($N>5000$), the Anderson-Darling tests requires a minimum of 7 test cases. 

Assessment for normality should never solely rely on p-values, but should be complemented by visual inspections

Therefore the function generates diagnostic plots: standardised residuals overlaid by the standard normal distribution, a scatter plot of the standardised residuals against the fitted means of the linear model for each level of the predictor, and a Q-Q plot of the standardised residuals.


The algorithmic decision logic cannot replicate the combination of formal tests and expert visual assessment of normality.
It therefore assumes normality if the Shapiro–Wilk test yields a p-value greater than $\alpha$, as simulation studies have shown it to offer the highest power against a variety of non-normal distributions in small to moderate samples [@Razali:2011].
<!-- Note: While Welch’s t-test omits normality checks for n > 30 based on the Central Limit Theorem (CLT), this rationale does not apply to ANOVA. Residuals from a fitted model are not independent and are influenced by the estimated parameters. Therefore, the CLT cannot be used to justify skipping the normality check, which is always applied to ANOVA residuals [@Lumley:2002]. -->
...
providing more reliable results in many practical situations.
...
However, simple pairwise comparisons of group means following an ANOVA the probability of incorrectly declaring a significant difference when, in fact, there is none.
...
Increasing the confidence level `conf.level` from the default 0.9 to 0.99 results in wider confidence intervalls of the regression paramters as well as wider confidence and prediction bands

```{r,fig.show='hide'}
linreg_trees_99 <- visstat(trees$Girth, trees$Volume,conf.level = 0.99)
```

Use `visStatistics` plot method to only display again the seond generated plot (the assumption plot is unchanged):
...
## Limitationof of decision-logic based on p-values
...
visual inspection of sample distributions provided by `visstat()`. Based on these plots, it may be necessary to a override the automated choice of test in individual cases.
