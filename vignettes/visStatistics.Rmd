...
(Eq.\ \@ref(eq:simple-regression-fit)).
...
# Test selection based on the general linear model framework {#sec:assumptions}
...
$\beta_0$ and $\beta_1$ of the linear regression model in
Eq.\ \@ref(eq:glm) with one predictor.
It is displayed with pointwise confidence and prediction bands at the
...
determination $R^2$ (Eq.\ \@ref(eq:r-squared)) as effect size.
...
After test selection, `visstat()` also reports an `effect_size` field for the
selected test. Effect sizes complement p-values because they describe the
magnitude of the observed difference or association, whereas statistical
significance is strongly affected by sample size [@Fritz:2012].
