...
`"feature"` must be  character strings that match column names in the `dataframe`.
...
If both the predictor and the response are numerical and contain only one level each, `visstat()` performs a simple linear regression.
