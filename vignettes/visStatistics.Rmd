...
`visstat(dataframe,  "response",  "predictor")`

`dataframe` must be a column-based `data.frame`. Both `response` and
`predictor` are character strings that must match column names in the `dataframe`.
