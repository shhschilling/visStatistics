...
<!-- normality, CLT  -->
By the Central Limit Theorem, the sampling distribution of the group means approaches normality as the group sizes $n_i \to \infty$, regardless of the shape of the underlying distribution. For finite $n_i$ this holds only approximately, but it implies that in large enough samples a rejected residual-normality test no longer threatens the validity of the mean-based test.
...
When this bypass fires, the routing has not yet tested homoscedasticity, so its justification must hold for both mean-based families it might reach: the homoscedastic comparison---Student's t-test, generalised to Fisher's one-way ANOVA---and the heteroscedastic comparison---Welch's t-test, generalised to Welch's one-way ANOVA. 


Published Monte Carlo studies examine both against a common criterion: under the null hypothesis of equal population means, and at a significance level of $\alpha = 5\%$, the empirical Type I error rate must stay within Bradley's liberal robustness bounds of $[2.5\%, 7.5\%]$ [@Bradley:1978].

Their shared finding is that, for up to four groups, group sizes around 50 keep both families within these bounds as long as the distributions are no more than moderately skewed (skewness up to 2): 
<!-- homoscedastic comparison -->
For the homoscedastic comparison,
Blanca et al. [@Blanca:2017] showed in a study of three groups that the Type I error of Fisher's  one-way ANOVA stayed within Bradley's bounds in every condition examined---skewness up to 2 and kurtosis up to 6, at group sizes from 5 to 100 [@Blanca:2017]; its two-group special case is Student's t-test (Eq.\ \@ref(eq:fisher-f)), for which the margin is wider still---across skewness up to 4 and moderate differences in spread, the two-sample t-test's rejection rate stayed between 4\% and 5\% for sample sizes from 10 upward [@Fagerland:2012].

<!-- heterocedastic comparison -->
For the heteroscedastic comparison, Welch's one-way ANOVA---with Welch's t-test as its two-group special case---holds the rate at about 50 observations for up to four group comparisons.
Beyond four groups, the cutoff of 50 holds unless skewness exceeds about 2---a pronounced asymmetry---where Welch's ANOVA needs roughly 100 per group [@Delacre:2019].
...
merely routes homoscedastic data to Welch's t-test or Welch's one-way ANOVA, which lose only negligible power to Student' t-test or Fisher Oneway Anova, when variances are in fact equal. In the case of balanced designs and equal variances,  Welch's t-test is  algebraicallly equivalent to Student's t-test (eq.)  and  Welch's one-way ANOVA converges asymptotically to the Fisher One Way Anova. (eq. )
 <!-- Why levene  -->

Some authors therefore recommend them as the default [@Rasch:2011; @Delacre:2019].
