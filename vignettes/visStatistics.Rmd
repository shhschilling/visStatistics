...
The Shapiro--Wilk (SW) normality test functions as the residual-normality gate, because among the normality tests compared by @Razali:2011 it is the most powerful against both symmetric and asymmetric alternatives, although all of them have low power below about 30 observations.
...
Under `group_test = "welch"` the analysis remains mean-based and uses Welch-type tests.
A Welch default avoids the type I error introduced by preliminary variance tests [@Moser:1992; @Zimmerman:2004; @Rochon:2012]. It gives directly interpretable estimates and confidence intervals; by the Central Limit Theorem it also remains valid in large samples even when residual normality is rejected, although how large is large enough depends on skewness and tail weight [@Lumley:2002]. It can be weak for strongly skewed small samples [@Blanca:2017; @Fagerland:2012] and can answer the wrong scientific question when the mean lies in a long tail [@Fagerland:2009]. The cost of defaulting to Welch-type mean comparisons is small in the equal-variance balanced case: Welch's t-test then coincides with Student's t-test, and for more than two balanced groups the Welch ANOVA statistic converges to Fisher's statistic with a relative difference of order $n^{-1}$ (Eqs. \@ref(eq:welch-student-pooled) and \@ref(eq:welch-anova-equal-var-reduction)). 
...
Under `group_test = "rank"` the analysis remains rank-based.
 defaulting to the `wilcox.test()` in the two group case and the `kruskal.test()` (`K`) for more than two groups. This reduces distributional assumptions and can be efficient under skew [@Bridge:1999], but changes the estimand [@Fagerland:2009] and gives less direct mean-scale interpretation.
...
Tests selection based on assumption tests suffers from the fact that all hypothesis tests are underpowered in smaller samples, thus often not detecting assumptions violations, and overpowered in large samples [@Ghasemi:2012; @Fagerland:2012; @Shatz:2024; @Knief:2021], resulting that in larger samples even mildly skewed data will be sent to the rank base route. 
...
Whereas the type I error rate from `SW` ranges from 4.4% to 5.6% in all simulated set-ups, the rejection rate from `SW+L` is only slightly higher, increasing from  4.5% to 5.7%, thus the difference is within the Monte Carlo standard error.


## Equal means, introducing heteroscedastici
...
Albeit Fisher's Anova  (`F`) assumes variance homogeneity, it remains within Bradley's boundaries for all simulated balanced, unequal standard deviation four group commparison (panel B of Fig. \@ref(fig:route1-unequal-typeI)) as well in unbalanced simulations where  smaller group sizes where combined with smaller standard deviaons (panel C). 

This is not the case anymore in the reversed scenario (panel D):  Combining larger groups with smaller standard deviations , `F` rejects the null hypotheses of equal means between between  12.1%--13.1%. In this reversed scenario, also `W` leaves the Bradley bounds only for skewness 2 with excess kurtosis 6.6 for a mean group size smaller than \bar n=20$ (panel D).

In the first two symmetric columns of Fig. \@ref(fig:route1-unequal-typeI)), both `SW` and `SW+L` are still type I tests and remain in Bradleys boundaries in the balanced simulations (panel B) as well as the unbalanced simulations of panel C. But combining smaller sample sizes with larger standard deviations (panel D), `SW` leaves the Bradley bound in three out of 8 scenarios with $\bar n>50$, as in  larger samples the Shapiro--Wilk gate is overpowered and sends most replications to `KW`, whose own rate is affected by unequal variances at unequal group sizes [@Zimmerman:2004]. Adding the Levene gate (`SW+L`) increases the type I error in smaller samples even further, as the Levene gate at small $n$ is underpowered  at $\bar n=10$ and $\bar n=20$ and routes nearly half the replications to `F`. `SW+L` leaves the bounds in six of eight cells of panel D , at worst at about twice the nominal 5%.
...
However, no single assumption test maintains optimal Type I error rates and statistical power across all distributions [@Olejnik:1987] and sample sizes.
In large samples, even minor deviations reach significance, so that at large $N$ the gate routes even mildly skewed data to the rank branch; in small samples, substantial violations do not, so that at small $N$ it is underpowered and keeps moderate skew on the mean route [@Kozak:2018].
Thus, the robustness of statistical tests depends on both the sample size and the shape of the underlying distribution.

`visstat()` routes on a residual-normality p-value, a sample-size-dependent proxy for the shape that actually governs whether the mean is a faithful estimand.
...
The Shapiro--Wilk statistic is an omnibus normality test, sensitive to both skewness ($\sqrt{b_1}$) and kurtosis ($b_2$) though built from neither [@DAgostino:1973]; asymmetry is the easier departure to detect at equal kurtosis [@Razali:2011].
The Shapiro--Wilk test is used because it is the transparent diagnostic already displayed, not because it is the optimal selection statistic.

Delacre et al. [@Delacre:2019] recommend Welch's test as the default whenever means are compared, whereas Bridge and Sawilowsky [@Bridge:1999] recommend the rank test whenever the shape of the population is unknown.
`visstat()` exposes both as explicit settings and, in the default, decides between them from the residual diagnostics it displays.

Under `group_test = "welch"` a Type I error of the variance gate is of no real consequence: it merely routes homoscedastic data to Welch's t-test or Welch's one-way ANOVA, which lose only negligible power relative to Student's t-test or Fisher's one-way ANOVA when variances are in fact equal, and under balanced designs with equal variances Welch's t-test is algebraically equivalent to Student's t-test and Welch's one-way ANOVA converges to Fisher's [@Rasch:2011; @Delacre:2019].

Under `group_test = "rank"` the distributional assumptions are dropped and little power is lost under normality, while power is gained under skew [@Bridge:1999], at the price of a different estimand [@Fagerland:2009].

Under the default, the Shapiro--Wilk test on the standardised residuals responds to both skewness and kurtosis, so heavily skewed or heavy-tailed data are not forced into a mean comparison, while the general linear model is retained when its two displayed assumptions hold; under normality the general-linear-model tests are uniformly most powerful unbiased [@Bridge:1999; @Knief:2021] and yield the regression and ANOVA decomposition that the rank route does not provide.
The case against gating is not answered by this design: Knief and Forstmeier [@Knief:2021] find Gaussian models robust across a wide range of non-normal conditions and warn that the alternatives carry risks that are less well known, and conditioning the choice on a preliminary test distorts the error rate of the procedure as a whole [@Rochon:2012; @Zimmerman:2004].
Section \@ref(sec:simulation-results) shows where that cost falls for each of the three options.

What the default adds is therefore not accuracy but visibility: the diagnostics that drive the choice are displayed, the selected test is named, and the configuration in which the choice is unreliable raises a warning.
Users who want the target fixed in advance can set `group_test` and obtain either recommendation from the literature directly.
