...
To stabilise the variance, we transform the count data by the square root.
...
`visstat()` checks the normal distribution of the standardised residuals derived from `lm()` both graphically and with the Shapiro-Wilk and Anderson test (analogue to section [ANOVA](#aov). If the p-values of the null that the standardised residuals are normally distributed of both Shapiro-Wilk and Anderson test are smaller than 1-`conf.int`, the title of the residual plot will display the message "Requirement of normally distributed residuals not met".
...
Pearson's Chi-squared on this 2 x2 contingency table would lead to a  expected value smaller than 5 in one of the 4 cells (corresponding to 25% of all cells) violating the requirement of the Chi-squared test  that the expected counts should be at least 5 for the majority (80%) of the cells [@Cochran]. Therefore the Fisher  exact test is selected . 
