...
If the feature named `varfactor` consists of data of `class` `"factor"` with two or more levels and the response named `varsample` consists of data of `class` `"numeric"` or `"integer"`, tests are applied to compare the means or medians.

## Two sample tests: Welch's t-test or Wilcoxon rank-sum test

If the feature named `varfactor` consists of data of `class` `"factor"` with exactly two levels and the response named `varsample` is numerical, either Welch's t-test or the Wilcoxon rank-sum test are performed. The choice follows the algorithm below:
...
The graphical representation consists of box plots overlayed with jitter plots showing each data point. In the case of the `t-test.()`, additionally the`conf.level\`$\cdot 100 \%$ -confidence intervals are shown.The test statistics of the chosen test as well as the summary statistics of the generated box plots is generated and returned as a list.
...
In the graphical representation of One-way test and ANOVA, the green letters between two levels only differ, if the Tukey's HSD corrected p-value for those two levels is smaller than `1-confint`.

#### Post-hoc analysis: Sidak corrected confidence intervals

Tukey\`s HSD procedure is based on **pairwise** comparisons of the differences between the means per factor level, whereas the Sidak procedure counteracts the problems of a type I error by lowering the acceptable probability of a type I error for **all** comparisons of the levels of the independent, categorical variable.
...
The `npk` data sets reports the yield of peas in pounds/block on 6 blocks, where the usage of the fertilizers nitrogen (N), phosphate (P) or potassium (K) was varied. Either no, one, two or three different were applied on the blocks.
