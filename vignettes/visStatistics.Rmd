...
When selecting tests for group comparisons, packages with overlapping scope such as `automatedtests` [@Zeevat:2025] and `compareGroups` [@Subirana:2014] automate test choice by evaluating normality per group.
...
It recommends exploring alternatives outside `visstat()` such as data transformations, [generalised linear models](#gamma-glm), or robust regression (see [Limitations](#limitations)).

Within `visstat()`, Spearman rank correlation offers an assumption-free alternative, at the cost of causal interpretability:
...
<a id="gamma-glm"></a>
To maintain a causal relationship, we model Ozone directly using a Gamma GLM with log link, which addresses the right-skewed distribution of Ozone concentrations.
...
To verify the model fit, we check the standardized deviance residuals. If the Gamma model correctly represents the data structure, these residuals should be approximately normally distributed. In our example, both the Shapiro-Wilk and Anderson-Darling normality tests on the standardized residuals confirm that this diagnostic requirement is met
