...
<!-- Section \@ref(sec:route-1) described how `visstat()` chooses: in the default setting `group_test = NULL` the residual diagnostics decide, whereas the two overrides `group_test = "welch"` and `group_test = "rank"` fix the choice in advance. The simulations below quantify what each of these three settings costs. -->

Under `group_test = "welch"` the analysis remains mean-based and defaults to Welch-type tests.
<!-- A Welch default avoids the type I error introduced by preliminary variance tests [@Moser:1992; @Zimmerman:2004; @Rochon:2012]. It gives directly interpretable estimates and confidence intervals; by the Central Limit Theorem it also remains valid in large samples even when residual normality is rejected, although how large is large enough depends on skewness and tail weight [@Lumley:2002]. It can be weak for strongly skewed small samples [@Blanca:2017; @Fagerland:2012] and can answer the wrong scientific question when the mean lies in a long tail [@Fagerland:2009]. -->


<!-- One sentene why no ham to default to W when F is allowed  -->
...
<!-- Route 1 -->
For tests of central tendency, p-values from assumption tests of normality and homoscedasticity of the model residuals are used as routing criteria in the default setting.
These p-values respond not only to the shape of the input samples, but also to the sample size and design balance (Section \@ref(sec:simulation-results)).

Mean tests within the general linear model framework are only retained when both the assumption of residual normality (assessed by the Shapiro–Wilk test) and residual homoscedasticity (assessed by the Levene test) are met. These tests are, under normality, uniformly most powerful  [@Bridge:1999]. If the assumptions of parametric tests are violated, `visstat()`  falls back to non-parametric tests in the default setting (corresponding to `SW+L`). 
...
Assessing assumptions solely through p-values can lead to both type I errors (false positives) and type II errors (false negatives) and no single assumption test maintains optimal Type I error rates and statistical power across all distributions [@Olejnik:1987] and sample sizes, and p-values obtained from these tests may be unreliable if their assumptions are violated. 

<!--- Limitation of automated routing in Route 1 -->
...
<!--- Consequences or type I error in Shapiro test   -->
A type I error in the Shapiro--Wilk residual-normality gate sends input that satisfies the general linear model assumptions to the rank branch. In our simulations, under true normality, it occurs  only at the nominal level of about $\alpha$ of all analyses (column 1 of Fig. \@ref(fig:route1-identical-typeI)). Since that input is normal and homoscedastic, the Kruskal--Wallis null is true as well and the general rejection rate remains close to the  nominal level. The real impact is changing the question answered from mean comparison to rank comparisons. 

<!--- Consequences or type I error in Levene test   -->
...
E.g., in our simulations we observe a type II error in normality testing  at small sample sizes of 10 in each symmetric (skew=0), heavy tailed (excess kurtosis=6) group: The default routing (`SW+L`) still sends 51% of the replications to Fisher's one-way ANOVA and a further 5% to Welch's ANOVA, albeit the normality assumption of the residuals is not given. The type II error in normality testing vanishes with growing sample size: at 100 observations per group the same shape is diverted in 99.99% of the replications to the Kruskal-Wallis test (column 2, row `SW+L` of panel B, Fig. \@ref(fig:route1-identical-typeI)).

<!--- Type 2 error in levene    -->
...
<!--- change in nominal level to test concatenation    -->
...
<!--- Arguments for the avoidance of assumptions    -->
...
<!-- Why welch default in two groups? -->

Fagerland and Sandvik [@Fagerland:2009] compared five two-sample location tests --- Student's t-test, Welch's t-test, the Yuen--Welch test on trimmed means, the Wilcoxon rank-sum test and the Brunner--Munzel test --- varying unequal variances and skewness , and found that the significance level and power depend on an interplay of skewness, skewness heterogeneity, variance heterogeneity, sample size and sample size ratio.
They recommend the Welch test more often than any of the other four, but note that the Welch test is  sensitive to skewness  - not astounding, as  the assumption of all Welch tests is normality in the groups. 
...
This scope keeps the decision tree inspectable and the graphical output consistent, but it also leaves several modelling choices (e.g. paired tests, interaction terms, multiple linear regression, generalised linear models, robust regression) outside the automated workflow.
 <!-- each requires user judgment -- about the transformation family, the link function, or the estimator -- that cannot be automated without substantially expanding the decision tree and increasing the risk of Type I error inflation. -->
...
Its design is therefore a combination of both "assumption checking" [@Shatz:2024] by visualisation and "assumption testing" by p-values.
