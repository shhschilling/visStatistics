...
Its main assumption-driven branch concerns tests of central tendency for a numeric response grouped by a factor. There, p-values from tests on pooled standardised residuals from a linear model fitted to all groups guide the choice between rank-based, equal-variance, and Welch-type procedures.
...
[@Games:1976] for more than two groups.
...
```{r ordinal-kruskal-example, fig.cap=paste0("Kruskal-Wallis test for ordered comfort ratings by customer segment. The ordered response is converted to integer level codes; box plots show the Kruskal-Wallis result; Holm-adjusted pairwise Wilcoxon post-hoc comparisons are shown as significant-letter labels ($\\alpha = ", example_alpha, "$)."), out.width="48%", fig.height=4.5, fig.show="hold", results='hide'}
set.seed(123)
