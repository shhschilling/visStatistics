...
Note that in the backward-compatible form second argument belongs to the response, the third to the predictor.
...
-   Normality testing of the residuals: A linear model `lm(y~x)` is fitted to extract standardized residuals, which are assessed for normality using the Shapiro-Wilk (SW) test [@Shapiro:1965] (`shapiro.test()`) at significance level $\alpha$.
...
-   Regardless of sample size, general linear model (GLM) assumption diagnostics are always displayed.
    When heteroscedasticity is detected and therefore Welch methods are selected, group-wise normality diagnostics are additionally displayed because residual-based normality testing assumes homoscedasticity.
...
The diagnostic plots are enhanced by p-values of tests for normality and homoscecadicity, shown in the title of the plot.
...
Note that when the assumption of equal variances holds ($s_1^2 = s_2^2 = s^2$) and sample sizes are equal ($n_1 = n_2 = n$), Welch's t-test reduces to Student's t-test with $2n - 2$ degrees of freedom.
...
If the predictor is of `class` "`factor`" with **more than two levels** and the response is of `mode` "`numeric`", `visstat()` either performs 
Fisher’s one- way ANOVA [@Fisher:1935] (`aov()`), 
Welch’s heteroscedastic one-way ANOVA [@Welch:1951] (`oneway.test()`) or, as a non-parametric alternative, the Kruskal -Wallis test [@Kruskal:1952] (`kruskal.test()`).
...
#### Connection to Fisher's one-way ANOVA
...
If both the predictor and the response are numeric vectors, `visstat()` performs a simple linear regression.
...
If both variables are numeric vectors (not data frames / matrices), `visstat()` computes correlation coefficients to measure the strength and direction of their linear or monotonic relationship, if the flag `do_regression=FALSE` is set.
...
When the response variable is an ordered factor (e.g., Likert scale ratings) of class `ordered` and the predictor is categorical, `visstat()` converts the ordered response to numeric ranks and defaults to non-parametric tests.
...
This package uses $n > 50$ per group as a rule of thumb threshold to omit normality testing, assuming parametric tests  for central tendencies become robust to non-normality at larger sample sizes based on the central limit theorem.
