...
-   If the sample size of both samples is bigger than 100, the function `visstat()` performs always **Welch's t-Test** (`t.test()`) [@Lumley:2002dsa ].
    ...
    The graphical representation names the used test, the underlying null hypothesis with the associated p-value of the null hypothesis. A box plot with an overlayed jitter plot showing each data point is generated. The number of data entry of each sample is displayed above each box. The means and Sidak corrected confidence intervals (see section \@ref(sidak)) are displayed in red.  
...
-   If the sample size of one of the samples is smaller than 100, `viststat()` checks for the normality of the standardized residuals of both samples with the Shapiro-Wilk Normality Test (`shapiro.test()`). If the normality assumption is met, Welch's t-Test is performed, otherwise the Wilcoxon rank sum test. The detailed algorithm is:
    ...
    The graphical representation is analogue to Welch's t-test omiting mean and the  confidence intervals. The test statistics of the Wilcoxon rank sum test as well as the summary statistics of the generated boxplots is generated and returned as a list.
...
-   If the p-value of the `bartlett.test()` is bigger than `1-conf.level`, we can assume homogeneity of variances in each level (group) and the p-values of `aov()` is displayed in the title.

-   Otherwise homoscedacity can not be assumed and the p-value of `oneway.test()` is displayed.

To generate Sidak corrected confidence intervals, we calculate the corrected probability of error for the construction of M confidence intervals, 
$$\alpha_{Sidak}=1-conf.int^{1/M}.$$ 
Here, $M$ is the number of levels of the independent variable [@Sidak].
Inserting the corrected $\alpha_{Sidak}$ into the standard formula for confidence intervals we obtain
...
## Transforming contingency tables: `counts_to_cases()`
...
HairEyeColorDataFrame=counts_to_cases(as.data.frame(HairEyeColor)) \#\# Examples
