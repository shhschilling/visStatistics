...
Both `aov()` and `oneway.test()` check, whether two or more samples from normal distributions have the same mean. Whereas `aov()` requires jomogenity of variances in each level (group), the `oneway.test()` does not assume that the variances in each group are necessarily equal.
