...
If the feature `varfactor` has more than two levels and  the response `varsample` is of type numerical or integer, in a first step 
the function `vis_anova_assumptions()` checks for normality of the standardised residuals of the
anova. It plots the standardized residuals versus the fitted mean values of the linear model for each level of `varfactor`. Furthermore it shows the  normal QQ plot of the standardized residuals. 
Additionally the p-values of  Shapiro-Wilk-test `shapiro.test()` and the Anderson-Darling-Test `ad.test()` are displayed.

`aov` further tests the homoscedacity of each factor level in `fact` with the `bartlett.test`.

aov$fitted are the fitted mean values.
## Examples
```{r}
 anova_npk=visstat(npk,"yield","block")
```

# Overview implemented tests
