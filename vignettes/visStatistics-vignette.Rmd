...
If the p-values of the standardized residuals calculated by `shapiro.test()`  or `ad.test()` are bigger than the error probability `1-conf.level`, both `aov()` and `oneway.test()` are executed and the corresponding p-values displayed.
...
The generated graphs shows in blue the mean plus/minus the  standard devation ofo each group. To generate the Sidak corrected confidence intervals in red, we entered the applied corrected probability of error for the comparison of $M$ groups,
