...
of the presented raw data. The  R package `visStatistics` with its core function `visstat()` fulfils this need. 
...
It  checks for the normality of the standardised residuals of the analysis of
variance (ANOVA) fit generated by the function `aov()` . To do so it applies, both the Shapiro-Wilk-test `shapiro.test()` and the Anderson-Darling-Test `ad.test()` on the standarized residuals. 
Furthermore the data is graphically analysed by plotting
...
The generated graphs shows in blue the mean plus/minus the  standard deviation ofo each group. To generate the Sidak corrected confidence intervals in red, we entered the applied corrected probability of error for the comparison of $M$ groups,

$\alpha_{Sidak}=1-$`conf.int`$^{1/M}$, [@Sidak] 
in the standard formula for confidence intervals
$$ \left[{{\overline {x}}-t_{\left(1-{\frac {\alpha_{Sidak} }{2}};n-1\right)}{\frac {s}{\sqrt {n}}}\ ;\ {\overline {x}}+t_{\left(1-{\frac {\alpha_{Sidak} }{2}};n-1\right)}{\frac {s}{\sqrt {n}}}}\right],
$$ 
...
# Linear Regression
If the feature `varfactor` and the response `varsample` have only one level of type 
numerical or integer, `visstat()` performs a linear regression. In the title of the graphical output the chosen confidence level `conf.level`, the regression parameter with their corresponding confidence levels and p-values as well as the adjusted $R^2$ are given. 
The graph shows the raw data, the regression line and both the confidence and prediction bands corresponding to the chosen `conf.level`. To check the underlying normality assumption of the linear regression, the p-values of `shapiro.test()`and `ad.test()` testing the null that the standardized residuals are returned. The function returns a list with the test statistics of the linear regression, the p-values of the normality tests and the pointwise estimates of the confidence and prediction band.

<!-- https://www.graphpad.com/guides/prism/latest/curve-fitting/reg_confidence_and_prediction_linear.htm -->
...
<!-- <!-- https://www.graphpad.com/guides/prism/latest/curve-fitting/reg_confidence_and_prediction_linear.htm --> -->
<!-- "The goal of a prediction band is to cover with a prescribed probability the values of one or more future observations from the same population from which a given data set was sampled. Just as prediction intervals are wider than confidence intervals, prediction bands will be wider than confidence bands." -->
<!-- The 95% prediction band is the area in which you expect 95% of all data points to fall -->
...
```
The linear regression model explains 64% of the total variance of the dependent variables "Distance". p-values smaller than `conf.level` in both  Anderson-Darling normality test and the Shapiro-Wilk test of the standardized residuals sugget that the normality assumption underlying the linear regression is not met. 
