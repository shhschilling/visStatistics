...
When choosing the test, none of these packages checks normality on the residuals of the fitted linear model (each observation minus its predicted value).

Instead, `autotestR` and `boxTest` test the response within each group separately, while `automatedtests` and `compareGroups` test the response variable as a whole, ignoring the grouping.


Within-group testing not only mistargets the linear-model normality assumption but also runs one assumption test per group, inflating the chance of a spurious non-normality verdict as the number of groups grows; a single test on the pooled residuals avoids both problems
...
The purpose  of this vignette is two-fold: On the one hand side it explains  the
...
On the other side it serves as a text-book like reference of all implmented
tests, correlationd and effect-sizes (Appendices \@ref(sec:tests)--\@ref(sec:effect-size)),
thus that the user easily understands the output with out having to consult the
code or  literature.
...
The variance $\sigma^2$ represents the variation of the data on the regression , $Var(Y_i)=\sigma^2$, as both the (unknown) model parameters as well as predictors are not random. 
...
\hat{y}_i = b_0 + b_1 x_{i1} + \cdots +
      b_{k-1} x_{i,k-1}
\end{equation}
 with the estimates values $b_0, b_1, \ldots, b_{k-1}$ for the unknown model parameters $\beta_0, \beta_1, \ldots, \beta_{k-1}$. 
...
The residual standard error $SE_\text{res}$ is a *global* estimate for the unknown $\sigma$, but $SE_\text{res}^2$ is not the best estimate for the variance of the *individual* residual $\operatorname{Var}(e_i)$. It can be shown [@Cook:1982, p. 14; @Schutzenmeister:2012a, p. 145] that
...
where the leverage $h_i$ of observation $i$ is the $i$-th diagonal element $h_{ii}$ of the hat matrix [@Schutzenmeister:2012a, p. 144] -- the matrix that maps the observed values onto the fitted values $\hat{y}_i$ -- and measures how strongly the predictor values of observation $i$ influence its own fitted value. Equation \@ref(eq:var-leverage) shows that the raw residuals carry an unequal, leverage-dependent variance even when the errors are homoscedastic: observations with higher leverage have a smaller individual residual variance. Internally studentised ("standardised") residuals correct for this artefact. Dividing $e_i$ by its estimated individual standard error gives

\begin{equation}
r_i =\frac{e_i}{\sqrt{ SE_\text{res}^2\,(1-h_i)}}=
\frac{z_i}{\sqrt{1-h_i}}.
(\#eq:standardised)
\end{equation}
...
There, `visStatistics` uses its package implementation `bp.test()` of the Breusch--Pagan test  [@Breusch:1979] (Eq. \@ref(eq:breusch-pagan-bp)) on raw residuals [@Kozak:2018; @Schutzenmeister:2012a]. 
...
Since algorithmic logic based on p-values of assumptions tests, cannot replace expert visual judgment,  
...
For numeric responses with categorical predictors (Route 1), the diagnostic panel displays the residual histogram, the normal Q--Q plot, and the absolute standardised residuals $|z_i|$ (Eq. \@ref(eq:standardised)) by group.
The last panel shows whether residual spread is comparable across factor levels, the pattern assessed formally by the Levene and Bartlett variance checks.

For Route 3 (simple linear regression), the diagnostic panel displays the residual histogram with normal density overlay, the normal Q--Q plot, both computed from standardised residuals, and z-scaled residuals versus fitted values. (Eq. \@ref(eq:leverage-simple-regression) with Cook's distance contours computed on the same z-residual scale (see Section \@ref(sec:cooks-distance)).
...
When all group-specific sample sizes are greater than 50, the sampling distribution of the group means is treated as sufficiently close to normal by the central limit theorem [@Lumley:2002]. In this case, the test selection is independent of the residual-normality p-values, albeit the corresponding p-values area still reported and displayd in the diagnostic panels (Setion \@ref(sec:graphical)).
...
The Gamma family is suited here because Ozone is strictly positive and continuous, and its variance grows with the fitte values — the structure detected by the Breusch--Pagan test.
...
Many routine statistical analysis reduce to a relatively small number of tests. Under those tests parametric tests like t-tests, analysis of variance or regression belong to the family of general linear models, which's  assumptions are frequently not tested at all or not tested properly [@Hoekstra:2012; @Ernst:2017; @Jones:2025; @Kery:2003].


`visStatistics`  addresses this gap: Its automated test selection relying on the data type, size and p-values of assumption tests of the model residuals  is fully transparent, but <span style="color:red">addresses</span> the inherent shortcomings of test selection based on the p-values by assumptions tests <span style="color:red">[@Lumley:2002; @Fagerland:2012; @Franc:2025; @Kozak:2018; @Shatz:2024]</span>  by supplementing the output with diagnostic plots of the assumption tests. 
