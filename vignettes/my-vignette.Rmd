...
This vignette  documents the algorithm underlying the decision tree of `vistat()`. It  depends on the type of the  data (numerical/integer or categorical), the number of levels of categorical data and the distribution of the samples. 
With the exception of `conf.level`, all underlying statistical tests called by the decision tree are used  with their default parameter sets; whilst 
the confidence level `conf.level`, defaulting to 0.95, can be adapted by the user  calling the core function `visstat()`.
...
Al underlying statistical functions are called  with their default parameters. Only the parameter `conf.level`, defaulting to 0.95, can be adapted by the user. 
...
* If the sample size of both samples is bigger than 100, the function `visstat()` performs always **Welch's t-Test** (`t.test()`) [@Lumley:2002dsa ]. 
    ...
    **Graphical representation**
    ...
    The title names the used test, the underlying null hypothesis with the associated p-value of null hypothesis. 
