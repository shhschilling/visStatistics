...
Statistical consulting  requires often both a quick visualization and reproducible statistical analysis of the presented raw data. The package `visStatistics` with its core function `vistat()` fulfils this need. Based on a decision test it picks the 
...
a graph displaying the raw data accordingly to the chosen test is generated. This fully automated workflow is especially suited for browser based interfaces to server-based deployments of R. 
