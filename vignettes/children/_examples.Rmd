...
```
The user can eforce the Welch type analysis, by setting  `group_test="Welch`, this changes the assumption panel from the GLM assumption panel to normality tests by density plots and QQ plots for each group.


```{r welch-anova-assumption-example, fig.cap=paste0("Enforcing Welch tests changes the assumption panel to normality density  and Q-Q plots for each group, The test statistics are Shapiro-Wilk (SW) and Anderson-Darling (AD) normality tests. "), out.width="80%", fig.height=4.5, fig.keep="last"}
...
#### Kruskal--Wallis rank sum test with Dunn post-hoc comparisons
...
Since Shapiro--Wilk falls below $\alpha$, `visstat()` switches to `kruskal.test()` followed by Holm-adjusted `dunn.test()`.
...
With three predictor groups, `visstat()` routes to `kruskal.test()` followed by Holm-adjusted `dunn.test()`.
...
ordinal_wilcoxon_kruskal_cap <- paste0(
 ...
 "(right). ",
 "Holm-adjusted Dunn post-hoc comparisons are shown as ",
